...
test('clearing chat removes conversation and active request state', () => {
  ...
  assert.deepEqual(chatReducer(active, { type: 'chat.clear' }), initialChatState)
})

test('source links allow only approved Eastman HTTPS hosts', () => {
  assert.ok(safeEastmanUrl('https://www.eastman.com/en/products/product-finder'))
  assert.equal(safeEastmanUrl('http://www.eastman.com/product'), null)
  assert.equal(safeEastmanUrl('https://www.eastman.com.example.test/product'), null)
})
