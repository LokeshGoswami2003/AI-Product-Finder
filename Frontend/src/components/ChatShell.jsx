          ...
          <header className="widget-header">
            <div className="assistant-identity">
              ...
              <div>
                ...
                <span className={`assistant-status assistant-status--${status}`}>
                  <i aria-hidden="true" /> {statusLabel}
                </span>
              </div>
            </div>
            <div className="widget-actions">
              <button type="button" onClick={clearChat} aria-label="Start a new chat" title="New chat">
              ...
              <button type="button" onClick={() => setIsOpen(false)} aria-label="Minimize product finder" title="Minimize">
                <svg viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M5 12h14" />
                </svg>
              </button>
          ...
          <div className="widget-body">
            {state.messages.length === 0 && (
              <section className="chat-welcome" aria-labelledby="welcome-title">
                ...
                <p>
                  Describe your application or name a product. I’ll search the Eastman catalog and show the most relevant options.
                </p>
              ...
              <div className="message-list">
                {state.messages.map((message) => (
                  <article className={`message message--${message.role}`} key={message.id}>
                    <span className="sr-only">{message.role === 'user' ? 'You' : 'Product finder'}:</span>
                    ...
                    <div className="message-bubble">
                      ...
                      {message.sources?.length > 0 && (
                        <details className="sources">
                          <summary>{message.sources.length} official {message.sources.length === 1 ? 'source' : 'sources'}</summary>
                          <ul>
          ...
          <footer className="widget-composer">
            <form className="composer" onSubmit={(event) => { event.preventDefault(); submit() }}>
              ...
              {state.activeRequestId ? (
                <button className="send-button stop-button" type="button" onClick={cancel} aria-label="Stop response">
                  <span aria-hidden="true" />
                </button>
              ) : (
                <button className="send-button" type="submit" disabled={!draft.trim() || status !== 'connected'} aria-label="Send message">
                  <svg viewBox="0 0 24 24" aria-hidden="true">
            ...
            <div className="widget-note">
              <span>Verify product details before use.</span>
              <button type="button" onClick={onLogout}>End preview</button>
            </div>
          </footer>
        </aside>
      ) : (
        <button className="chat-launcher" type="button" onClick={() => setIsOpen(true)} aria-label="Open Eastman product finder">
          <span className="launcher-mark" aria-hidden="true">✦</span>
