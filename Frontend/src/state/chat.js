...
export function chatReducer(state, action) {
  switch (action.type) {
    case 'request.started':
      return {
        ...state,
        messages: [...state.messages, { id: action.requestId, role: 'user', content: action.message }],
    ...
    case 'answer.delta': {
      ...
      const existing = state.messages.find((message) => message.id === answerId)
      const messages = existing
        ...
        : [
            ...state.messages,
            { id: answerId, role: 'assistant', content: action.delta, sources: [], products: [] },
          ]
    ...
    case 'answer.products':
      return updateAnswer(state, action.requestId, { products: action.products })
    case 'answer.done':
