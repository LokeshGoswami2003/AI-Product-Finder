...
const envSchema = z
  .object({
    ...
    CHAT_MODE: z.enum(["offline", "bedrock"]).default("offline"),
    BEDROCK_API_KEY: z.string().min(20).optional(),
  ...
  .superRefine((environment, context) => {
    if (environment.CHAT_MODE === "bedrock" && !environment.BEDROCK_API_KEY) {
      context.addIssue({
        code: "custom",
        path: ["BEDROCK_API_KEY"],
        message: "BEDROCK_API_KEY is required when CHAT_MODE=bedrock",
      });
    }
  });
