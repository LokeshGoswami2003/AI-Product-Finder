...
const { PARSER_VERSION, SCHEMA_VERSION, normalizeCatalog } = require('./normalize')
...
function validateNormalizedCatalog(normalized, expectedProductCount) {
  ...
  const identifiers = new Set(normalized.products.map((product) => product.fgmn))
  ...
  for (const product of normalized.products) {
    for (const url of Object.values(product.links)) {
      if (!validateEastmanUrl(url)) {
        throw new Error(`Product ${product.fgmn} contains a non-allowlisted URL`)
      }
    }
  }
...
async function buildCatalogRelease({
  ...
  const productCount = expectedProductCount ?? catalog.productDetails.labels.totalCount
