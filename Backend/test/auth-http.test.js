...
const { createSessionToken, verifySessionToken } = require('../src/auth/session')
...
test('signed session tokens reject tampering and expiration', () => {
  const token = createSessionToken({ secret: config.COOKIE_SIGNING_SECRET, ttlSeconds: 10, now: 0 })
  assert.ok(verifySessionToken(token, { secret: config.COOKIE_SIGNING_SECRET, now: 9_000 }))
  assert.equal(verifySessionToken(`${token}x`, { secret: config.COOKIE_SIGNING_SECRET }), null)
  assert.equal(
    verifySessionToken(token, { secret: config.COOKIE_SIGNING_SECRET, now: 10_000 }),
    null,
  )
})
...
test('access-code login requires exact origin and creates an authenticated session', async () => {
  ...
  await agent.post('/api/auth/logout').set('Origin', config.APP_ORIGIN).expect(204)
